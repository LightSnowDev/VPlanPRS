...
import com.lightSnow.VPlanPRS.DownloadHelper;
...
public class RegistrationIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        try {
            ...
            StorageHelper.saveToSharedPreferences(StorageHelper.SENT_TOKEN_TO_SERVER, true, this);
            // [END register_for_gcm]
        } catch (Exception e) {
            Log.d(TAG, "Failed to complete token refresh", e);
            // If an exception happens while fetching the new token or updating our registration data
            // on a third-party server, this ensures that we'll attempt the update at a later time.
            StorageHelper.saveToSharedPreferences(StorageHelper.SENT_TOKEN_TO_SERVER, false, this);
        }
        // Notify UI that registration has completed, so the progress indicator can be hidden.
        Intent registrationComplete = new Intent(StorageHelper.REGISTRATION_COMPLETE);
    ...
    private void sendRegistrationToServer(String token, Context context) {
        ...
        //security
        token = "" + token + "";
        sendGCMServerData(SensibleDataHelper.serverPathFile + token, context);
    }
    ...
    private void sendGCMServerData(String urlString, Context context){
        ...
        downloadHelper.addOnRecievedEvent(new DownloadHelper.OnRecievedEvent() {
            ...
            public void recievedEvent(String result, boolean success) {
                Log.d("GCM event", "token send to server: " +  String.valueOf(success));
            }
        });
        downloadHelper.download(urlString, SensibleDataHelper.serverAccountName, SensibleDataHelper.serverAccountPassword);
    }
